...
from   dustUI                     import dustWindow,                 \
                                         ...
                                         dustFrameText
# from    pdUI                        import pdWindow,pdFrameApi,pdFrameConnection,pdFrameCommand,pdFrameResponse,pdFrameNotification,pdFrameText
import csv
#============================ logging =========================================
...
class APIExplorer(object):
    ...
    def _apiFrameCb_apiLoaded(self,apiDefLoaded):
        ...
        #self.connectionFrame._connect_xml()
        #self.connector.send(['sendRequest'],{'priority': 'low', 'domain': 'maintenance', 'macAddr': '00-17-0D-00-00-58-F3-12', 'data': [0, 0, 252, 18, 171], 'reliable': True})
            ...
            if returnVal:
                if returnVal[0] == ['data']:
                    ...
                    with open('data.csv', 'a') as csvfile:
                        fieldnames = ['macAddr', 'time','payload','callbackId','isRequest','counter','isReliable','payloadType','isBroadcast','moteId']
                        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
                        writer.writeheader()
                        writer.writerow(returnVal[1])
            return returnVal
